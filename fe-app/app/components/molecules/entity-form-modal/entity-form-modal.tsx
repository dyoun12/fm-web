    ...
    >
      <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl">
          ...
          <dl className="mt-6 grid gap-4 rounded-2xl border border-zinc-100 bg-zinc-50/70 p-4 sm:grid-cols-2">
            {readOnlyFields.map((field) => (
              <div key={field.label}>
                ...
                <dd className="mt-1 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-700">{field.value ?? "-"}</dd>
              </div>
            ))}
          </dl>
        )}
        ...
        <form className="mt-6 grid gap-4" onSubmit={onSubmit}>
          {children}
          <div className={cn("mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end", onDelete && "sm:justify-between")}>
            {onDelete && (
              <Button
                type="button"
                variant="ghost"
