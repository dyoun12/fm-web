...
### 1) 요구사항/계약 정리
- [ ] `docs/prd.md`의 연락처 페이지 설명에 관리자 저장 및 이메일 발송 요구사항 반영
- [ ] `docs/spec.md`에 `/api/contact` → `/v1/contact` 플로우 및 바디 스키마 정의
- [ ] `docs/backend.md`에서 문의 API 구현 시 고려할 연동/에러 모델 검토
- [ ] `docs/dynamodb-schema.md`에 `ContactInquiry` 엔티티 구조 추가

### 2) 백엔드 구현
- [ ] `be-app/app/api/routers/contact.py`(또는 기존 라우터)에 `POST /v1/contact` 추가
- [ ] 요청 DTO/응답 DTO 정의(회사명, 직책, 이름, 이메일, 유입경로, 제목, 내용, 생성일시 등)
- [ ] `ContactInquiry` 저장 로직 구현(DynamoDB 단일 테이블 또는 기존 테이블)
- [ ] 회사 대표 이메일 조회(`corpmeta.email`/`CorpInfo.email`) 헬퍼 구현
- [ ] 메일 발송 구현(AWS SES 등), 실패 시 재시도/로그 전략 정의
- [ ] 백엔드 단위 테스트/통합 테스트 추가(Pytest)

### 3) 프론트엔드 구현(문의 폼 제출)
- [ ] `fe-app/api/contact.ts` (또는 기존 API 레이어)에 `postContactInquiry` 함수 추가
- [ ] `/api/contact` Route Handler에서 백엔드 `/v1/contact` 호출 연동
- [ ] `ContactForm`/`/contact` 페이지에서 `onSubmit`을 통해 API 호출하도록 연결
- [ ] 제출 성공/실패 상태에 따라 UI 메시지/상태(로딩, Disabled, 에러) 처리
- [ ] 프론트엔드 단위 테스트 추가(Vitest + Testing Library, MSW로 네트워크 모킹)

### 4) 관리자 콘솔 연동
- [ ] 관리자 라우트(예: `/admin/contact`) 추가 및 네비게이션에 노출
- [ ] 문의 목록 테이블(접수일시, 이름, 이메일, 제목, 상태) 구현
- [ ] 행 클릭 시 상세 보기(회사명, 직책, 유입경로, 전체 메시지) 모달/페이지 구현
- [ ] 처리 상태 변경 기능(옵션: `PATCH /v1/contact/{inquiryId}`) 및 토스트 알림
- [ ] 관리자 화면 테스트 추가(렌더/상호작용 스모크)
...
- [ ] 본 태스크 파일(`tasks/contact-inquiry-flow.md`) 체크리스트 및 관련 PR/스크린샷 링크 업데이트
