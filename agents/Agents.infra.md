...
- 인프라 중심: VPC/서브넷/보안그룹/VPC 엔드포인트/공통 S3/ECR/IAM 등 관리
- 서버리스 앱 배포: AWS SAM으로 API Gateway/Lambda/DynamoDB 빌드/배포/테스트 자동화
...
2) Terraform: VPC/서브넷/보안그룹, VPC 엔드포인트, 공통 S3/ECR/IAM 등 프로비저닝
3) SAM: API Gateway/Lambda/DynamoDB 빌드(`sam build`) → 배포(`sam deploy`) → 로컬 검증(`sam local start-api`)
